...
bool wav2vec2_stt_init(const char *model_dirname) {
...
bool wav2vec2_stt_decode(float *wav_samples, int32_t wav_samples_len, char *text, int32_t text_max_len) {
    BEGIN_INTERFACE_CATCH_HANDLER
    auto options = torch::TensorOptions().dtype(torch::kFloat32);
    auto wav_tensor = torch::from_blob(wav_samples, {wav_samples_len}, options);
